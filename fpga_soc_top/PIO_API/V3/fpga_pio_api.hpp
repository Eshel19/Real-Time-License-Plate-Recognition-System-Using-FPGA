...
#endif
#ifndef PIO_STATUS_BASE
#  define PIO_STATUS_BASE 0xC0000020u       
#endif
...
// ─────────────────── 32‑bit generic PIO register ───────────────────
class PioReg32 {
public:
    explicit PioReg32(std::uintptr_t base,
                      std::chrono::milliseconds = 0ms) : reg_(base) {}

    inline void      writeWord(uint32_t v,
                               std::size_t off = 0) const noexcept {
        reg_.write<uint32_t>(v, off);
    }
    inline uint32_t  readWord(std::size_t off = 0) const noexcept {
        return reg_.read<uint32_t>(off);
    }
